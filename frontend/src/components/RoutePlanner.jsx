...
const RoutePlanner = () => {
  ...
  return (
    <div className="flex flex-col h-full bg-slate-950 overflow-hidden font-sans">
      ...
      <header className="p-6 bg-slate-900 border-b border-white/5 flex items-center justify-between shadow-xl z-20">
        <div className="flex items-center gap-8">
          <div className="flex items-center gap-4">
            <div className="space-y-1">
              <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest block">Start Station</label>
              <select 
                value={startNode} onChange={e => setStartNode(e.target.value)}
                className="bg-slate-800 border border-slate-700 rounded-xl px-4 py-2 text-sm outline-none focus:ring-1 ring-emerald-500 transition-all text-white w-48"
              >
                {HUBS.map(hub => <option key={hub} value={hub}>{hub}</option>)}
              </select>
            </div>
            <ChevronRight className="w-4 h-4 text-slate-600 self-end mb-3" />
            <div className="space-y-1">
              <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest block">Destination</label>
              <select 
          ...
          <div className="space-y-1 w-64">
            <div className="flex justify-between items-center mb-1">
              <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest block">Rainfall Saturation</label>
        ...
        <button 
          ...
          className="bg-emerald-600 hover:bg-emerald-500 disabled:bg-slate-700 transition-all px-8 py-3 rounded-2xl flex items-center gap-3 shadow-lg shadow-emerald-900/20"
        >
      ...
      <div className="flex-1 relative flex">
        ...
        {routeData && (
          <aside className="absolute top-6 right-6 z-[1000] w-96 glass-panel p-6 rounded-3xl border border-white/10 shadow-2xl space-y-6 animate-in fade-in slide-in-from-right-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="text-sm font-black text-white uppercase tracking-widest">Route Risk Audit</h3>
                <p className="text-[10px] text-slate-500 font-bold uppercase tracking-tight">OSRM Geographic Analysis</p>
              </div>
            </div>

            <div className="grid grid-cols-2 gap-4">
               <div className="p-4 rounded-2xl bg-slate-800/40 border border-white/5">
                  <p className="text-[9px] font-black text-slate-500 uppercase tracking-widest mb-1">Peak Hazard Score</p>
                  <p className={`text-2xl font-black ${routeData.max_risk > 65 ? 'text-red-400' : 'text-emerald-400'}`}>{routeData.max_risk}%</p>
               </div>
               <div className="p-4 rounded-2xl bg-slate-800/40 border border-white/5">
                  <p className="text-[9px] font-black text-slate-500 uppercase tracking-widest mb-1">Data Segments</p>
                  <p className="text-2xl font-black text-slate-100">{routeData.path.length}</p>
               </div>
            </div>
            ...
            <div className={`p-4 rounded-2xl border ${routeData.max_risk > 65 ? 'bg-red-500/10 border-red-500/20' : 'bg-emerald-500/10 border-emerald-500/20'} space-y-2`}>
              ...
              <p className="text-xs text-slate-300 leading-relaxed italic font-medium">"{routeData.safety_briefing}"</p>
            </div>
          </aside>
        )}
      </div>
